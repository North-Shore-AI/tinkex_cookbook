defmodule TinkexCookbook.Adapters.TrainingClient.Tinkex do
  ...
  @impl true
  @spec forward_backward(keyword(), session(), list()) :: Task.t()
  def forward_backward(_adapter_opts, %{id: training_client}, datums) do
    # Convert datums to Tinkex format
  ...
  @doc """
  Get the tokenizer from a training session.

  This is a Tinkex-specific operation not part of the standard port behaviour.
  """
  @spec get_tokenizer(session()) :: {:ok, term()} | {:error, term()}
  def get_tokenizer(%{id: training_client}) do
  ...
  # Convert CrucibleTrain.Types.Datum to Tinkex format
  defp datum_to_tinkex(%CrucibleTrain.Types.Datum{} = datum) do
  ...
  defp model_input_to_tinkex(%CrucibleTrain.Types.ModelInput{chunks: chunks}) do
    tinkex_chunks =
      Enum.map(chunks, fn
        %CrucibleTrain.Types.EncodedTextChunk{tokens: tokens} ->
  ...
  defp tensor_data_to_tinkex(%CrucibleTrain.Types.TensorData{} = td) do
